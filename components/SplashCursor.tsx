...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function initFramebuffers() {
      ...
      if (!dye)
        dye = createDoubleFBO(
          dyeRes.width,
          dyeRes.height,
          (rgba ?? fallbackFormat).internalFormat,
          (rgba ?? fallbackFormat).format,
          texType,
          filtering
        );
      else
      ...
      if (!velocity)
        velocity = createDoubleFBO(
          simRes.width,
          simRes.height,
          (rg ?? fallbackFormat).internalFormat,
          (rg ?? fallbackFormat).format,
          texType,
          filtering
        );
      else
      ...
      divergence = createFBO(
        simRes.width,
        simRes.height,
        (r ?? fallbackFormat).internalFormat,
        (r ?? fallbackFormat).format,
        texType,
        gl.NEAREST
      );
      curl = createFBO(
    ...
    function resizeCanvas() {
      ...
      let width = scaleByPixelRatio((canvas as HTMLCanvasElement).clientWidth);
      let height = scaleByPixelRatio((canvas as HTMLCanvasElement).clientHeight);
    ...
    function generateColor(): { r: number; g: number; b: number } {
      // Avoid very light colors: use medium to medium-dark, vivid colors
      const h = Math.random();
      const s = 0.7 + Math.random() * 0.3; // 0.7-1.0
      const v = 0.3 + Math.random() * 0.3; // 0.3-0.6
    ...
    window.addEventListener('mousemove', (e) => {
      const now = Date.now();
      if (now - lastSplatTime > SPLAT_THROTTLE_MS) {
        lastSplatTime = now;
        let pointer = pointers[0];
        let posX = scaleByPixelRatio(e.clientX);
        let posY = scaleByPixelRatio(e.clientY);
